<?php
class Mnj_submenu extends CI_Controller{
	   ...
	   $cek = $this->session->userdata('logged_in');
		...
		if(!empty($cek)){
			...
			$d['listjns'] = $this->app_model->manualQuery($jns);
       		     ...
       		     $this->template->display('mnj_submenu/view',$d);
		}else{
  ...
  $where ="";
   $searchName = isset($_GET['nm_menu']) ? $_GET['nm_menu']: false;
     $searchLengkap = isset($_GET['nm_submenu']) ? $_GET['nm_submenu']: false;
       $searchCabang = isset($_GET['linksub']) ? $_GET['linksub']: false;
          $searchNIP = isset($_GET['ket']) ? $_GET['ket']: false;
    ...
    if ($_GET['_search'] == 'true') {
        ...
        if(!empty($searchName)) {
                 $menu = $this->app_model->CariParameterLike($searchName,'nm_menu','tb_menu','id_menu');
        
          $searchString.="id_menuutama like '%$menu%'";
          }
       if(!empty($searchNIP)) {
          if(empty($searchString)){
            $searchString.="ket like '%$searchNIP%'";
            }else{
                 $searchString.="AND ket like '%$searchNIP%'";
          ...
          if(empty($searchString)){
            $searchString.="linksub like '%$searchCabang%'";
            }else{
                 $searchString.="AND linksub like '%$searchCabang%'";
    ...
    if(!$sidx)
        $sidx =1;
    $count = $this->db->count_all_results('tb_submenu');
    if( $count > 0 ) {
    ...
    if ($page > $total_pages)
        $page=$total_pages;
      $query = $this->app_model->getAllbar($start,$limit,$sidx,$sord,$where,'tb_submenu');
    ...
    foreach($query as $row) {
                $row->nm_menu = $this->app_model->CariParameter($row->id_menuutama,'id_menu','tb_menu','nm_menu');
       
                   $row->detail = " <a href='javascript:tampil_data(\"{$row->id_submenu}\")' class='btn btn-xs btn-warning'><i class='glyphicon glyphicon-edit' title='EDIT'></i></a> <a href='javascript:hapusData(\"{$row->id_submenu}\")' class='btn btn-xs btn-danger'><i class='glyphicon glyphicon-trash' title='Hapus'></i></a>";

         $responce->rows[$i]['id']=$row->id_submenu;
        $responce->rows[$i]['cell']=array($row->nm_menu,$row->nm_submenu,$row->linksub,$row->ket,$row->detail);
        $i++;
    }
		...
		if(!empty($cek)){
          ...
          $d['title']="Managemen SUB MENU";
		   ...
		   $d['listjenis'] = $this->app_model->manualQuery($jns_pembiayaan);
               ...
               $biaya = "SELECT * FROM tb_submenu where id_submenu='".$idd."'";
			$d['listbiaya'] = $this->app_model->manualQuery($biaya);

            	 		$d['biayadata'] = $this->app_model->CariAll($idd,'tb_submenu','id_submenu');
			...
			$d['data'] = $this->app_model->manualQuery($text);
             ...
             $qcabang = "SELECT * FROM tb_menu order by id_menu";
         ...
         $this->load->view('mnj_submenu/view_data',$d);
    	...
    	function hapus()
	{
		...
		if(!empty($cek)){
			...
			$this->app_model->manualQuery("DELETE FROM tb_submenu WHERE id_submenu='$id'");
  	}else{
			header('location:'.base_url());
		}
	}
      	...
      	public function simpan()
	{
		...
		if(!empty($cek)){
           $this->output->cache(1);
               ...
               $up['nm_submenu']=$this->input->post('nm_submenu');
                	$up['linksub']=$this->input->post('linksub');
					$up['ket']=$this->input->post('ket');
                    $up['id_menuutama']=$this->input->post('id_menuutama');
              	$username=$this->input->post('id_submenu');
                     	$id['id_submenu']=$this->input->post('id_submenu');
				$data = $this->app_model->CariAll($username,'tb_menu','id_menu');
				if($data==1){
					$this->app_model->updateData("tb_submenu",$up,$id);

				}else{
					$this->app_model->insertData("tb_submenu",$up);
