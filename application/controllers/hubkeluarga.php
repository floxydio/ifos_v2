<?php
class Hubkeluarga extends CI_Controller{
	   ...
	   $cek = $this->session->userdata('logged_in');
		...
		if(!empty($cek)){
			...
			$d['listjns'] = $this->app_model->manualQuery($jns);
       		     ...
       		     $this->template->display('hubkeluarga/view',$d);
		}else{
   ...
   public function loadData(){
    ...
    $sidx = isset($_GET['sidx'])?$_GET['sidx']:'id_nasabah';
  ...
  $where ="";
   $searchName = isset($_GET['nm_nasabah']) ? $_GET['nm_nasabah']: false;
    ...
    if ($_GET['_search'] == 'true') {
        ...
        if(!empty($searchName)) {
          ...
          $searchString.="nm_nasabah like '%$searchName%'";
          }
    ...
    if(!$sidx)
        $sidx =1;
    $count = $this->db->count_all_results('tb_hubnasabah');
    if( $count > 0 ) {
    ...
    if ($page > $total_pages)
        $page=$total_pages;
    $query = $this->app_model->getAllbar($start,$limit,$sidx,$sord,$where,'tb_hubnasabah');
    $responce->page = $page;
    $responce->total = $total_pages;
    $responce->records = $count;
    $i=0;
    foreach($query as $row) {
                     $row->detail = " <a href='javascript:tampil_data(\"{$row->id_nasabah}\")' class='btn btn-xs btn-warning'><i class='glyphicon glyphicon-edit' title='EDIT'></i></a> <a href='javascript:hapusData(\"{$row->id_nasabah}\")' class='btn btn-xs btn-danger'><i class='glyphicon glyphicon-trash' title='Hapus'></i></a>";

         $responce->rows[$i]['id']=$row->id_nasabah;
        $responce->rows[$i]['cell']=array($row->nm_nasabah,$row->detail);
        $i++;
    }
           	...
           	function tambahdata($idd)
	{
		...
		if(!empty($cek)){
          ...
          $d['title']="Managemen Hubungan Keluarga";
           $d['tes']="ok";
		   ...
		   $d['listjenis'] = $this->app_model->manualQuery($jns_pembiayaan);
               ...
               $biaya = "SELECT * FROM tb_hubnasabah where id_nasabah='".$idd."'";
			$d['listbiaya'] = $this->app_model->manualQuery($biaya);

            	$d['biayadata'] = $this->app_model->CariAll($idd,'tb_hubnasabah','id_nasabah');
                                	$nn = "SELECT * FROM users where username='$nm_account' ORDER BY username ASC ";
			$d['listname'] = $this->app_model->manualQuery($nn);
                 	$ww = "SELECT * FROM tb_cabang ";
			$d['listcabangbsm'] = $this->app_model->manualQuery($ww);
         ...
         $this->load->view('hubkeluarga/view_data',$d);
    	...
    	function hapus()
	{
		...
		if(!empty($cek)){
			...
			$this->app_model->manualQuery("DELETE FROM tb_hubnasabah WHERE id_nasabah='$id'");
  	}else{
			header('location:'.base_url());
		}
	}
      	...
      	public function simpan()
	{
		...
		if(!empty($cek)){
           $this->output->cache(1);
          		...
          		$up['nm_nasabah']=$this->input->post('nm_nasabah');

                	$id_nasabah=$this->input->post('id_nasabah');
                 	$id['id_nasabah']=$this->input->post('id_nasabah');

				$data = $this->app_model->CariAll($id_nasabah,'tb_hubnasabah','id_nasabah');
				if($data==1){
					$this->app_model->updateData("tb_hubnasabah",$up,$id);

				}else{
					$this->app_model->insertData("tb_hubnasabah",$up);
