...
class Konfirmasi extends CI_Controller{
    ...
    function __construct(){
   	...
   	function konfirmasi_parameter($del,$view)
	{
		...
		if(!empty($cek)){
			...
			$d['listjabatanbsm'] = $this->app_model->manualQuery($rr);
		     ...
		     $d['title']=$this->config->item('title');
			  $linksuub = "konfirmasi/konfirmasi_parameter/".$del."/view";
             $menu = $this->app_model->CariParameterLike($linksuub,'linksub','tb_submenu','id_menuutama');
			 $levelsub = $this->app_model->CariParameterLev($menu,$level);
			// $d['levels'] = $this->app_model->CariParameterLev($menu,$level);
		   //  $d['mens'] = $this->app_model->CariParameterLike($linksuub,'linksub','tb_submenu','id_menuutama');
			...
			if($levelsub=='0'){
			 	$text = "SELECT * FROM users where nm_cabang='$cabang' and level in ('0')";
			}else{
           	$text = "SELECT * FROM users where nm_cabang='$cabang' and level in ('".$levelsub."')";
			}
