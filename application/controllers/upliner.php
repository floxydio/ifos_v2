<?php
class Upliner extends CI_Controller{
	   ...
	   $cek = $this->session->userdata('logged_in');
		...
		if(!empty($cek)){
			...
			$d['listjns'] = $this->app_model->manualQuery($jns);
       		     ...
       		     $this->template->display('upliner/view',$d);
		}else{
  ...
  $where ="";
   $searchName = isset($_GET['userid']) ? $_GET['userid']: false;
     $searchLengkap = isset($_GET['nm_upliner']) ? $_GET['nm_upliner']: false;
    ...
    if ($_GET['_search'] == 'true') {
        ...
        if(!empty($searchName)) {
          ...
          $searchString.="userid like '%$searchName%'";
          }

          if(!empty($searchLengkap)) {
          if(empty($searchString)){
            $searchString.="nm_upliner like '%$searchLengkap%'";
            }else{
                 $searchString.="AND nm_upliner like '%$searchLengkap%'";
    ...
    if(!$sidx)
        $sidx =1;
    $count = $this->db->count_all_results('tb_upliner');
    if( $count > 0 ) {
    ...
    if ($page > $total_pages)
        $page=$total_pages;
    $query = $this->app_model->getAllbar($start,$limit,$sidx,$sord,$where,'tb_upliner');
    $responce->page = $page;
    ...
    foreach($query as $row) {
          ...
          $row->jabatan = $this->app_model->CariJabatan($row->id_jabatanpeg);
                   $row->detail = " <a href='javascript:tampil_data(\"{$row->id_upliner}\")' class='btn btn-xs btn-warning'><i class='glyphicon glyphicon-edit' title='EDIT'></i></a> <a href='javascript:hapusData(\"{$row->id_upliner}\")' class='btn btn-xs btn-danger'><i class='glyphicon glyphicon-trash' title='Hapus'></i></a>";

         $responce->rows[$i]['id']=$row->id_upliner;
        $responce->rows[$i]['cell']=array($row->userid,$row->nm_upliner,$row->detail);
        $i++;
    }
		...
		if(!empty($cek)){
          ...
          $d['title']="Managemen Upliner";
		   ...
		   $d['listjenis'] = $this->app_model->manualQuery($jns_pembiayaan);
               ...
               $biaya = "SELECT * FROM tb_upliner where id_upliner='".$idd."'";
			$d['listbiaya'] = $this->app_model->manualQuery($biaya);

            	$d['biayadata'] = $this->app_model->CariAll($idd,'tb_upliner','id_upliner');
                                	$nn = "SELECT * FROM users where username='$nm_account' ORDER BY username ASC ";
			$d['listname'] = $this->app_model->manualQuery($nn);
                 	$ww = "SELECT * FROM tb_cabang ";
			$d['listcabangbsm'] = $this->app_model->manualQuery($ww);
            ...
            $qupline = "SELECT * FROM users WHERE level IN ('3','4','0') ORDER BY username";
			$d['listupline'] = $this->app_model->manualQuery($qupline);

                	$text = "SELECT * FROM users where level IN ('4','0','2')";
			$d['listusersss'] = $this->app_model->manualQuery($text);


                $d['no_aplikasi'] = $this->app_model->MaxNoAplikasi();

         $this->load->view('upliner/view_data',$d);
    	...
    	function hapus()
	{
		...
		if(!empty($cek)){
			...
			$this->app_model->manualQuery("DELETE FROM tb_upliner WHERE id_upliner='$id'");
  	}else{
			header('location:'.base_url());
		}
	}
      	...
      	public function simpan()
	{
		...
		if(!empty($cek)){
           $this->output->cache(1);
               	...
               	$up['userid']=$this->input->post('userid');
				$up['nm_upliner']=$this->input->post('nm_upliner');

                	$username=$this->input->post('userid');
                 	$id['userid']=$this->input->post('userid');

				$data = $this->app_model->CariAll($username,'tb_upliner','userid');
				if($data==1){
					$this->app_model->updateData("tb_upliner",$up,$id);

				}else{
					$this->app_model->insertData("tb_upliner",$up);
