<?php
class Jnsbangunan extends CI_Controller{
	   ...
	   $cek = $this->session->userdata('logged_in');
		...
		if(!empty($cek)){
			...
			$d['listjns'] = $this->app_model->manualQuery($jns);
       		     ...
       		     $this->template->display('jnsbangunan/view',$d);
		}else{
   ...
   public function loadData(){
    ...
    $sidx = isset($_GET['sidx'])?$_GET['sidx']:'id_jnsbngn';
  ...
  $where ="";
   $searchName = isset($_GET['nm_jnsbngn']) ? $_GET['nm_jnsbngn']: false;
    ...
    if ($_GET['_search'] == 'true') {
        ...
        if(!empty($searchName)) {
          ...
          $searchString.="nm_jnsbngn like '%$searchName%'";
          }
    ...
    if(!$sidx)
        $sidx =1;
    $count = $this->db->count_all_results('tb_jnsbngn');
    if( $count > 0 ) {
    ...
    if ($page > $total_pages)
        $page=$total_pages;
    $query = $this->app_model->getAllbar($start,$limit,$sidx,$sord,$where,'tb_jnsbngn');
    $responce->page = $page;
    $responce->total = $total_pages;
    $responce->records = $count;
    $i=0;
    foreach($query as $row) {
                     $row->detail = " <a href='javascript:tampil_data(\"{$row->id_jnsbngn}\")' class='btn btn-xs btn-warning'><i class='glyphicon glyphicon-edit' title='EDIT'></i></a> <a href='javascript:hapusData(\"{$row->id_jnsbngn}\")' class='btn btn-xs btn-danger'><i class='glyphicon glyphicon-trash' title='Hapus'></i></a>";

         $responce->rows[$i]['id']=$row->id_jnsbngn;
        $responce->rows[$i]['cell']=array($row->nm_jnsbngn,$row->detail);
        $i++;
    }
           	...
           	function tambahdata($idd)
	{
		...
		if(!empty($cek)){
          ...
          $d['title']="Managemen Jenis Bangunan";
           $d['tes']="ok";
		   ...
		   $d['listjenis'] = $this->app_model->manualQuery($jns_pembiayaan);
               ...
               $biaya = "SELECT * FROM tb_jnsbngn where id_jnsbngn='".$idd."'";
			$d['listbiaya'] = $this->app_model->manualQuery($biaya);

            	$d['biayadata'] = $this->app_model->CariAll($idd,'tb_jnsbngn','id_jnsbngn');
                                	$nn = "SELECT * FROM users where username='$nm_account' ORDER BY username ASC ";
			$d['listname'] = $this->app_model->manualQuery($nn);
                 	$ww = "SELECT * FROM tb_cabang ";
			$d['listcabangbsm'] = $this->app_model->manualQuery($ww);
         ...
         $this->load->view('jnsbangunan/view_data',$d);
    	...
    	function hapus()
	{
		...
		if(!empty($cek)){
			...
			$this->app_model->manualQuery("DELETE FROM tb_jnsbngn WHERE id_jnsbngn='$id'");
  	}else{
			header('location:'.base_url());
		}
	}
      	...
      	public function simpan()
	{
		...
		if(!empty($cek)){
           $this->output->cache(1);
          		...
          		$up['nm_jnsbngn']=$this->input->post('nm_jnsbngn');

                	$id_jnsbngn=$this->input->post('id_jnsbngn');
                 	$id['id_jnsbngn']=$this->input->post('id_jnsbngn');

				$data = $this->app_model->CariAll($id_jnsbngn,'tb_jnsbngn','id_jnsbngn');
				if($data==1){
					$this->app_model->updateData("tb_jnsbngn",$up,$id);

				}else{
					$this->app_model->insertData("tb_jnsbngn",$up);
