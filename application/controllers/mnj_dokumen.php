<?php
class Mnj_dokumen extends CI_Controller{
	   ...
	   $cek = $this->session->userdata('logged_in');
		...
		if(!empty($cek)){
			...
			$d['listjns'] = $this->app_model->manualQuery($jns);
       		     ...
       		     $this->template->display('mnj_dokumen/view',$d);
		}else{
  ...
  $where ="";
   $searchName = isset($_GET['nm_dokumen']) ? $_GET['nm_dokumen']: false;
     $searchLengkap = isset($_GET['grup_peg']) ? $_GET['grup_peg']: false;
      $searchLahir = isset($_GET['mandatory']) ? $_GET['mandatory']: false;
    ...
    if ($_GET['_search'] == 'true') {
        ...
        if(!empty($searchName)) {
          ...
          $searchString.="nm_dokumen like '%$searchName%'";
          }

          if(!empty($searchLengkap)) {
          if(empty($searchString)){
            $searchString.="grup_peg like '%$searchLengkap%'";
            }else{
                 $searchString.="AND grup_peg like '%$searchLengkap%'";
          ...
          if(empty($searchString)){
            $searchString.="mandatory like '%$searchLahir%'";
            }else{
                 $searchString.="AND mandatory like '%$searchLahir%'";
    ...
    if(!$sidx)
        $sidx =1;
    $count = $this->db->count_all_results('tb_dokumen');
    if( $count > 0 ) {
    ...
    if ($page > $total_pages)
        $page=$total_pages;
    $query = $this->app_model->getAllbar($start,$limit,$sidx,$sord,$where,'tb_dokumen');
    $responce->page = $page;
    ...
    foreach($query as $row) {
          ...
          $row->jabatan = $this->app_model->CariJabatan($row->id_jabatanpeg);
                   $row->detail = " <a href='javascript:tampil_data(\"{$row->id_dokumen}\")' class='btn btn-xs btn-warning'><i class='glyphicon glyphicon-edit' title='EDIT'></i></a> <a href='javascript:hapusData(\"{$row->id_dokumen}\")' class='btn btn-xs btn-danger'><i class='glyphicon glyphicon-trash' title='Hapus'></i></a>";

         $responce->rows[$i]['id']=$row->id_dokumen;
        $responce->rows[$i]['cell']=array($row->nm_dokumen,$row->grup_peg,$row->mandatory,$row->detail);
        $i++;
    }
		...
		if(!empty($cek)){
          ...
          $d['title']="Managemen Dokumen";
		   ...
		   $d['listjenis'] = $this->app_model->manualQuery($jns_pembiayaan);
               ...
               $biaya = "SELECT * FROM tb_dokumen where id_dokumen='".$idd."'";
			$d['listbiaya'] = $this->app_model->manualQuery($biaya);

            	$d['biayadata'] = $this->app_model->CariAll($idd,'tb_dokumen','id_dokumen');
                                	$nn = "SELECT * FROM users where username='$nm_account' ORDER BY username ASC ";
			$d['listname'] = $this->app_model->manualQuery($nn);
                 	$ww = "SELECT * FROM tb_cabang ";
			$d['listcabangbsm'] = $this->app_model->manualQuery($ww);
            ...
            $qjabatan = "SELECT * FROM tb_grupdok";
         ...
         $this->load->view('mnj_dokumen/view_data',$d);
      	...
      	public function simpan()
	{
		...
		if(!empty($cek)){
           $this->output->cache(1);
                 	...
                 	$up['nm_dokumen']=$this->input->post('nm_dokumen');
                	$up['grup_peg']=$this->input->post('grup_peg');
                		$up['mandatory']=$this->input->post('mandatory');
                    
                    
					$username=$this->input->post('id_dokumen');
                     	$id['id_dokumen']=$this->input->post('id_dokumen');

			 	$data = $this->app_model->CariAll($username,'tb_dokumen','id_dokumen');

				if($data==1){
					$this->app_model->updateData("tb_dokumen",$up,$id);

				}else{
					$this->app_model->insertData("tb_dokumen",$up);
