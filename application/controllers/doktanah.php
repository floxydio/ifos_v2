<?php
class Doktanah extends CI_Controller{
	   ...
	   $cek = $this->session->userdata('logged_in');
		...
		if(!empty($cek)){
			...
			$d['listjns'] = $this->app_model->manualQuery($jns);
       		     ...
       		     $this->template->display('doktanah/view',$d);
		}else{
   ...
   public function loadData(){
    ...
    $sidx = isset($_GET['sidx'])?$_GET['sidx']:'id_doktanah';
  ...
  $where ="";
   $searchName = isset($_GET['nm_doktanah']) ? $_GET['nm_doktanah']: false;
    ...
    if ($_GET['_search'] == 'true') {
        ...
        if(!empty($searchName)) {
          ...
          $searchString.="nm_doktanah like '%$searchName%'";
          }
    ...
    if(!$sidx)
        $sidx =1;
    $count = $this->db->count_all_results('tb_doktanah');
    if( $count > 0 ) {
    ...
    if ($page > $total_pages)
        $page=$total_pages;
    $query = $this->app_model->getAllbar($start,$limit,$sidx,$sord,$where,'tb_doktanah');
    $responce->page = $page;
    $responce->total = $total_pages;
    $responce->records = $count;
    $i=0;
    foreach($query as $row) {
                     $row->detail = " <a href='javascript:tampil_data(\"{$row->id_doktanah}\")' class='btn btn-xs btn-warning'><i class='glyphicon glyphicon-edit' title='EDIT'></i></a> <a href='javascript:hapusData(\"{$row->id_doktanah}\")' class='btn btn-xs btn-danger'><i class='glyphicon glyphicon-trash' title='Hapus'></i></a>";

         $responce->rows[$i]['id']=$row->id_doktanah;
        $responce->rows[$i]['cell']=array($row->nm_doktanah,$row->detail);
        $i++;
    }
           	...
           	function tambahdata($idd)
	{
		...
		if(!empty($cek)){
          ...
          $d['title']="Managemen Dokumen Tanah";
           $d['tes']="ok";
		   ...
		   $d['listjenis'] = $this->app_model->manualQuery($jns_pembiayaan);
               ...
               $biaya = "SELECT * FROM tb_doktanah where id_doktanah='".$idd."'";
			$d['listbiaya'] = $this->app_model->manualQuery($biaya);

            	$d['biayadata'] = $this->app_model->CariAll($idd,'tb_doktanah','id_doktanah');
                                	$nn = "SELECT * FROM users where username='$nm_account' ORDER BY username ASC ";
			$d['listname'] = $this->app_model->manualQuery($nn);
                 	$ww = "SELECT * FROM tb_cabang ";
			$d['listcabangbsm'] = $this->app_model->manualQuery($ww);
         ...
         $this->load->view('doktanah/view_data',$d);
    	...
    	function hapus()
	{
		...
		if(!empty($cek)){
			...
			$this->app_model->manualQuery("DELETE FROM tb_doktanah WHERE id_doktanah='$id'");
  	}else{
			header('location:'.base_url());
		}
	}
      	...
      	public function simpan()
	{
		...
		if(!empty($cek)){
           $this->output->cache(1);
          		...
          		$up['nm_doktanah']=$this->input->post('nm_doktanah');

                	$id_doktanah=$this->input->post('id_doktanah');
                 	$id['id_doktanah']=$this->input->post('id_doktanah');

				$data = $this->app_model->CariAll($id_doktanah,'tb_doktanah','id_doktanah');
				if($data==1){
					$this->app_model->updateData("tb_doktanah",$up,$id);

				}else{
					$this->app_model->insertData("tb_doktanah",$up);
