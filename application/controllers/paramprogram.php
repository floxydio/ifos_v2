<?php
class Paramprogram extends CI_Controller{
	   ...
	   $cek = $this->session->userdata('logged_in');
		...
		if(!empty($cek)){
			...
			$d['listjns'] = $this->app_model->manualQuery($jns);
       		     ...
       		     $this->template->display('paramprogram/view',$d);
		}else{
   ...
   public function loadData(){
    ...
    $sidx = isset($_GET['sidx'])?$_GET['sidx']:'id_parameter';
  ...
  $where ="";
   $searchName = isset($_GET['id_program']) ? $_GET['id_program']: false;
     $searchLengkap = isset($_GET['nm_parameter']) ? $_GET['nm_parameter']: false;
      $searchMin = isset($_GET['range_min']) ? $_GET['range_min']: false;
      $searchMax = isset($_GET['range_max']) ? $_GET['range_max']: false;
    ...
    if ($_GET['_search'] == 'true') {
        ...
        if(!empty($searchName)) {
          ...
          $searchString.="id_program like '%$searchName%'";
          }

          if(!empty($searchLengkap)) {
          if(empty($searchString)){
            $searchString.="nm_parameter like '%$searchLengkap%'";
            }else{
                 $searchString.="AND nm_parameter like '%$searchLengkap%'";
             ...
             if(!empty($searchMax)) {
          if(empty($searchString)){
            $searchString.="range_max like '%$searchMax%'";
            }else{
                 $searchString.="AND range_max like '%$searchMax%'";
    ...
    if(!$sidx)
        $sidx =1;
    $count = $this->db->count_all_results('tb_parameterdek');
    if( $count > 0 ) {
    ...
    if ($page > $total_pages)
        $page=$total_pages;
    $query = $this->app_model->getAllbar($start,$limit,$sidx,$sord,$where,'tb_parameterdek');
    $responce->page = $page;
    ...
    foreach($query as $row) {
          ...
          $row->jabatan = $this->app_model->CariJabatan($row->id_jabatanpeg);
                   $row->detail = " <a href='javascript:tampil_data(\"{$row->id_parameter}\")' class='btn btn-xs btn-warning'><i class='glyphicon glyphicon-edit' title='EDIT'></i></a> <a href='javascript:hapusData(\"{$row->id_parameter}\")' class='btn btn-xs btn-danger'><i class='glyphicon glyphicon-trash' title='Hapus'></i></a>";

         $responce->rows[$i]['id']=$row->id_dokumen;
        $responce->rows[$i]['cell']=array($row->id_program,$row->nm_parameter,$row->range_min,$row->range_max,$row->detail);
        $i++;
    }
           	...
           	function tambahdata($idd)
	{
		...
		if(!empty($cek)){
          ...
          $d['title']="Managemen Parameter Program";
           $d['tes']="ok";
		   ...
		   $d['listjenis'] = $this->app_model->manualQuery($jns_pembiayaan);
               ...
               $biaya = "SELECT * FROM tb_parameterdek where id_parameter='".$idd."'";
			$d['listbiaya'] = $this->app_model->manualQuery($biaya);

            	$d['biayadata'] = $this->app_model->CariAll($idd,'tb_parameterdek','id_parameter');
                                	$nn = "SELECT * FROM users where username='$nm_account' ORDER BY username ASC ";
			$d['listname'] = $this->app_model->manualQuery($nn);
                 	$ww = "SELECT * FROM tb_cabang ";
			$d['listcabangbsm'] = $this->app_model->manualQuery($ww);
            ...
            $qjabatan = "SELECT * FROM tb_channel";
         ...
         $this->load->view('paramprogram/view_data',$d);
    	...
    	function hapus()
	{
		...
		if(!empty($cek)){
			...
			$this->app_model->manualQuery("DELETE FROM tb_dokumen WHERE id_dokumen='$id'");
  	}else{
			header('location:'.base_url());
		}
	}
      	...
      	public function simpan()
	{
		...
		if(!empty($cek)){
           $this->output->cache(1);
                 	...
                 	$up['nm_parameter']=$this->input->post('nm_parameter');
                	$up['id_program']=$this->input->post('id_program');
                		$up['range_min']=$this->input->post('range_min');
                    	$up['range_max']=$this->input->post('range_max');
                    
                    
					$id_parameter=$this->input->post('id_parameter');
                     	$id['id_parameter']=$this->input->post('id_parameter');

			 	$data = $this->app_model->CariAll($id_parameter,'tb_parameterdek','id_parameter');

				if($data==1){
					$this->app_model->updateData("tb_parameterdek",$up,$id);

				}else{
					$this->app_model->insertData("tb_parameterdek",$up);
