...
class Limit extends CI_Controller{
	   ...
	   $cek = $this->session->userdata('logged_in');
		...
		if(!empty($cek)){
			...
			$d['listjns'] = $this->app_model->manualQuery($jns);
       		     ...
       		     $this->template->display('limit/view',$d);
		}else{
  ...
  $where ="";
   $searchName = isset($_GET['username']) ? $_GET['username']: false;
     $searchLengkap = isset($_GET['jml_limit']) ? $_GET['jml_limit']: false;
    ...
    if ($_GET['_search'] == 'true') {
        ...
        if(!empty($searchName)) {
          ...
          if(empty($searchString)){
            $searchString.="limit like '%$searchLengkap%'";
            }else{
    ...
    if(!$sidx)
        $sidx =1;
    $count = $this->db->count_all_results('tb_limit');
    if( $count > 0 ) {
    ...
    if ($page > $total_pages)
        $page=$total_pages;
    $query = $this->app_model->getAllbar($start,$limit,$sidx,$sord,$where,'tb_limit');
    $responce->page = $page;
    $responce->total = $total_pages;
    $responce->records = $count;
    $i=0;
    foreach($query as $row) {
                  $row->detail = " <a href='javascript:tampil_data(\"{$row->id_limit}\")' class='btn btn-xs btn-warning'><i class='glyphicon glyphicon-edit' title='EDIT'></i></a> <a href='javascript:hapusData(\"{$row->id_limit}\")' class='btn btn-xs btn-danger'><i class='glyphicon glyphicon-trash' title='Hapus'></i></a>";

         $responce->rows[$i]['id']=$row->id_limit;
        $responce->rows[$i]['cell']=array($row->username,$row->jml_limit,$row->detail);
        $i++;
    }
		...
		if(!empty($cek)){
          ...
          $d['title']="Managemen Limit";
		   ...
		   $d['listjenis'] = $this->app_model->manualQuery($jns_pembiayaan);
               ...
               $biaya = "SELECT * FROM tb_limit where id_limit='".$idd."'";
			$d['listbiaya'] = $this->app_model->manualQuery($biaya);

            	$d['biayadata'] = $this->app_model->CariAll($idd,'tb_limit','id_limit');
                                	$nn = "SELECT * FROM users where username='$nm_account' ORDER BY username ASC ";
			$d['listname'] = $this->app_model->manualQuery($nn);
                 	$ww = "SELECT * FROM tb_cabang ";
			$d['listcabangbsm'] = $this->app_model->manualQuery($ww);
    	...
    	function hapus()
	{
		...
		if(!empty($cek)){
			...
			$this->app_model->manualQuery("DELETE FROM tb_limit WHERE id_limit='$id'");
  	}else{
			header('location:'.base_url());
		}
	}
      	...
      	public function simpan()
	{
		...
		if(!empty($cek)){
           $this->output->cache(1);
				...
				$up['jml_limit']=$this->input->post('jml_limit');

                	$id_limit=$this->input->post('id_limit');
                 	$id['id_limit']=$this->input->post('id_limit');

				$data = $this->app_model->CariAllTabel($id_limit,'tb_limit','id_limit');
				if($data==1){
					$this->app_model->updateData("tb_limit",$up,$id);

				}else{
					$this->app_model->insertData("tb_limit",$up);
