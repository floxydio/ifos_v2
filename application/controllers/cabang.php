...
class Cabang extends CI_Controller{
	   ...
	   $cek = $this->session->userdata('logged_in');
		...
		if(!empty($cek)){
			...
			$d['listjns'] = $this->app_model->manualQuery($jns);
       		     ...
       		     $this->template->display('cabang/view',$d);
		}else{
   ...
   public function loadData(){
    ...
    $sidx = isset($_GET['sidx'])?$_GET['sidx']:'kd_cabang';
  ...
  $where ="";
   $searchName = isset($_GET['kd_cabang']) ? $_GET['kd_cabang']: false;
     $searchCabang = isset($_GET['nm_cabang']) ? $_GET['nm_cabang']: false;
     $searchLahir = isset($_GET['kanwil']) ? $_GET['kanwil']: false;
    ...
    if ($_GET['_search'] == 'true') {
        ...
        if(!empty($searchName)) {
          ...
          $searchString.="kd_cabang like '%$searchName%'";
          }
   ...
   if(!empty($searchLahir)) {
          if(empty($searchString)){
            $searchString.="kanwil like '%$searchLahir%'";
            }else{
                 $searchString.="AND kanwil like '%$searchLahir%'";
    ...
    if(!$sidx)
        $sidx =1;
    $count = $this->db->count_all_results('tb_cabang');
    if( $count > 0 ) {
    ...
    if ($page > $total_pages)
        $page=$total_pages;
    $query = $this->app_model->getAllbar($start,$limit,$sidx,$sord,$where,'tb_cabang');
    $responce->page = $page;
    $responce->total = $total_pages;
    $responce->records = $count;
    $i=0;
    foreach($query as $row) {
                     $row->detail = " <a href='javascript:tampil_data(\"{$row->kd_cabang}\")' class='btn btn-xs btn-warning'><i class='glyphicon glyphicon-edit' title='EDIT'></i></a> <a href='javascript:hapusData(\"{$row->kd_cabang}\")' class='btn btn-xs btn-danger'><i class='glyphicon glyphicon-trash' title='Hapus'></i></a>";

         $responce->rows[$i]['id']=$row->kd_cabang;
        $responce->rows[$i]['cell']=array($row->kd_cabang,$row->nm_cabang,$row->kanwil,$row->detail);
        $i++;
    }
		...
		if(!empty($cek)){
          ...
          $d['title']="Managemen Cabang";
		   ...
		   $d['listjenis'] = $this->app_model->manualQuery($jns_pembiayaan);
               ...
               $biaya = "SELECT * FROM tb_cabang where kd_cabang='".$idd."'";
			$d['listbiaya'] = $this->app_model->manualQuery($biaya);

            	$d['biayadata'] = $this->app_model->CariAll($idd,'tb_cabang','kd_cabang');
                                	$nn = "SELECT * FROM users where username='$nm_account' ORDER BY username ASC ";
			$d['listname'] = $this->app_model->manualQuery($nn);
                 	$ww = "SELECT * FROM tb_cabang ";
			$d['listcabangbsm'] = $this->app_model->manualQuery($ww);
         ...
         $this->load->view('cabang/view_data',$d);
    	...
    	function hapus()
	{
		...
		if(!empty($cek)){
			...
			$this->app_model->manualQuery("DELETE FROM tb_cabang WHERE kd_cabang='$id'");
  	}else{
			header('location:'.base_url());
		}
	}
      	...
      	public function simpan()
	{
		...
		if(!empty($cek)){
           $this->output->cache(1);
          		...
          		$up['kd_cabang']=$this->input->post('kd_cabang');
          			$up['nm_cabang']=$this->input->post('nm_cabang');
          				$up['kanwil']=$this->input->post('kanwl');
             	$kd_cabang=$this->input->post('kd_cabang');
                 	$id['kd_cabang']=$this->input->post('kd_cabang');

				$data = $this->app_model->CariAll($kd_cabang,'tb_cabang','kd_cabang');
				if($data==1){
					$this->app_model->updateData("tb_cabang",$up,$id);

				}else{
					$this->app_model->insertData("tb_cabang",$up);
