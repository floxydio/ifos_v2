<?php
class Mnj_akses extends CI_Controller{
	   ...
	   $cek = $this->session->userdata('logged_in');
		...
		if(!empty($cek)){
			...
			$d['listjns'] = $this->app_model->manualQuery($jns);
       		     ...
       		     $this->template->display('mnj_akses/view',$d);
		}else{
  ...
  $where ="";
   $searchName = isset($_GET['nm_menu']) ? $_GET['nm_menu']: false;
     $searchLengkap = isset($_GET['nm_level']) ? $_GET['nm_level']: false;
    ...
    if ($_GET['_search'] == 'true') {
        ...
        if(!empty($searchName)) {
          ...
          $searchString.="id_menu like '%$menu%'";
          }
      
          if(!empty($searchLengkap)) {
          if(empty($searchString)){
			       $level = $this->app_model->CariParameterLike($searchLengkap,'nm_level','tb_level','id_level');
       
            $searchString.="level like '%$level%'";
            }else{
                 $searchString.="AND level like '%$level%'";
    ...
    if(!$sidx)
        $sidx =1;
    $count = $this->db->count_all_results('tb_menu_user');
    if( $count > 0 ) {
    ...
    if ($page > $total_pages)
        $page=$total_pages;
     $query = $this->app_model->getAllbar($start,$limit,$sidx,$sord,$where,'tb_menu_user');
    ...
    foreach($query as $row) {
           $row->nm_level = $this->app_model->CariParameter($row->level,'id_level','tb_level','nm_level');
            $row->nm_menu = $this->app_model->CariParameter($row->id_menu,'id_menu','tb_menu','nm_menu');
                    $row->detail = " <a href='javascript:tampil_data(\"{$row->id_muser}\")' class='btn btn-xs btn-warning'><i class='glyphicon glyphicon-edit' title='EDIT'></i></a> <a href='javascript:hapusData(\"{$row->id_muser}\")' class='btn btn-xs btn-danger'><i class='glyphicon glyphicon-trash' title='Hapus'></i></a>";

         $responce->rows[$i]['id']=$row->id_muser;
        $responce->rows[$i]['cell']=array($row->nm_menu,$row->nm_level,$row->detail);
        $i++;
    }
           	...
           	function tambahdata($idd)
	{
		...
		if(!empty($cek)){
          ...
          $d['title']="Managemen Level Akses Menu";
           $d['tes']="ok";
		   ...
		   $d['listjenis'] = $this->app_model->manualQuery($jns_pembiayaan);
               ...
               $biaya = "SELECT * FROM tb_menu_user where id_muser='".$idd."'";
			$d['listbiaya'] = $this->app_model->manualQuery($biaya);

            		$d['biayadata'] = $this->app_model->CariAll($idd,'tb_menu_user','id_muser');
                                	$nn = "SELECT * FROM users where username='$nm_account' ORDER BY username ASC ";
			$d['listname'] = $this->app_model->manualQuery($nn);
                 	$ww = "SELECT * FROM tb_cabang ";
			$d['listcabangbsm'] = $this->app_model->manualQuery($ww);
         ...
         $this->load->view('mnj_akses/view_data',$d);
    	...
    	function hapus()
	{
		...
		if(!empty($cek)){
			...
			$this->app_model->manualQuery("DELETE FROM tb_menu_user WHERE id_muser='$id'");
  	}else{
			header('location:'.base_url());
		}
	}
      	...
      	public function simpan()
	{
		...
		if(!empty($cek)){
           $this->output->cache(1);
               	...
               	$up['id_menu']=$this->input->post('id_menu');
                	$up['level']=$this->input->post('level');
                  
            
                	$username=$this->input->post('id_muser');
                     	$id['id_muser']=$this->input->post('id_muser');
				$data = $this->app_model->CariAll($username,'tb_menu_user','id_muser');
				if($data==1){
					$this->app_model->updateData("tb_menu_user",$up,$id);

				}else{
					$this->app_model->insertData("tb_menu_user",$up);
