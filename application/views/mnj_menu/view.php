<script type="text/javascript">
 ...
 function hapusData(kode){
	...
	if (pilih==true) {
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>mnj_menu/hapus",
			data	: string,
			cache	: false,
...
function tampil_data(dat){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>mnj_menu/tambahdata/"+dat,
			data	: string,
			cache	: false,
                beforeSend: function(){
...
<a href="<?php echo base_url();?>mnj_menu"  class="btn btn-primary"> <i class="glyphicon glyphicon-refresh"></i> Refresh</a>
<hr>
  ...
  <script type="text/javascript">
        ...
        $(document).ready(function (){
               jQuery("#list").jqGrid({
              	url		: "<?php echo site_url();?>mnj_menu/loadData",
                		styleUI : 'Bootstrap',
                 mtype : "get",
                datatype: "json",
               colNames:['Nama Menu','Link','Keterangan','Aksi'],
                 colModel:[
                   {name:'nm_menu',index:'nm_menu',key: true, width:170, align:"left"},
                    {name:'link',index:'link',key: true, width:150, align:"left"},
                    {name:'ket',index:'ket',key: true, width:150, align:"left"},
