...
function tampil_data(dat){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>program/tambahdata/"+dat,
			data	: string,
			cache	: false,
                beforeSend: function(){
...
<a href="<?php echo base_url();?>program"  class="btn btn-primary"> <i class="glyphicon glyphicon-refresh"></i> Refresh</a>
<hr>
  ...
  <script type="text/javascript">
        ...
        $(document).ready(function (){
               jQuery("#list").jqGrid({
              	url		: "<?php echo site_url();?>program/loadData",
                		styleUI : 'Bootstrap',
                 mtype : "get",
                datatype: "json",
               colNames:['Nama Channel','Aksi'],
                 colModel:[
                    {name:'nm_channel',index:'nm_channel',key: true, width:150, align:"left"},
