<script type="text/javascript">
 ...
 function hapusData(kode){
	...
	if (pilih==true) {
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>limit/hapus",
			data	: string,
			cache	: false,
...
function tampil_data(dat){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>limit/tambahdata/"+dat,
			data	: string,
			cache	: false,
                beforeSend: function(){
...
<hr>
  ...
  <script type="text/javascript">
        ...
        $(document).ready(function (){
               jQuery("#list").jqGrid({
              	url		: "<?php echo site_url();?>limit/loadData",
                		styleUI : 'Bootstrap',
                 mtype : "get",
                datatype: "json",
               colNames:['Userid','Nama Upliner','Aksi'],
                 colModel:[
                   {name:'username',index:'username',key: true, width:170, align:"left"},
                    {name:'jml_limit',index:'jml_limit',key: true, width:150, align:"left"},
