<script type="text/javascript">
 ...
 function hapusData(kode){
	...
	if (pilih==true) {
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>jnsbangunan/hapus",
			data	: string,
			cache	: false,
...
function tampil_data(dat){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>jnsbangunan/tambahdata/"+dat,
			data	: string,
			cache	: false,
                beforeSend: function(){
...
<a href="<?php echo base_url();?>jnsbangunan"  class="btn btn-primary"> <i class="glyphicon glyphicon-refresh"></i> Refresh</a>
<hr>
  ...
  <script type="text/javascript">
        ...
        $(document).ready(function (){
               jQuery("#list").jqGrid({
              	url		: "<?php echo site_url();?>jnsbangunan/loadData",
                		styleUI : 'Bootstrap',
                 mtype : "get",
                datatype: "json",
               colNames:['Nama Jenis Bangunan','Aksi'],
                 colModel:[
                    {name:'nm_jnsbngn',index:'nm_jnsbngn',key: true, width:150, align:"left"},
