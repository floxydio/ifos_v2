<script type="text/javascript">
 ...
 function hapusData(kode){
	...
	if (pilih==true) {
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>mnj_dokumen/hapus",
			data	: string,
			cache	: false,
...
function tampil_data(dat){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>mnj_dokumen/tambahdata/"+dat,
			data	: string,
			cache	: false,
                beforeSend: function(){
...
<a href="<?php echo base_url();?>mnj_dokumen"  class="btn btn-primary"> <i class="glyphicon glyphicon-refresh"></i> Refresh</a>
<hr>
  ...
  <script type="text/javascript">
        ...
        $(document).ready(function (){
               jQuery("#list").jqGrid({
              	url		: "<?php echo site_url();?>mnj_dokumen/loadData",
                		styleUI : 'Bootstrap',
                 mtype : "get",
                datatype: "json",
               colNames:['Nama Dokumen','Grup Dokumen','Mandatory','Aksi'],
                 colModel:[
                   {name:'nm_dokumen',index:'nm_dokumen',key: true, width:170, align:"left"},
                    {name:'grup_peg',index:'grup_peg',key: true, width:150, align:"left"},
                       {name:'mandatory',index:'mandatory',key: true, width:150, align:"left"},
