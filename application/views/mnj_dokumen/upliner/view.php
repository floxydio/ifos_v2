<script type="text/javascript">
 ...
 function hapusData(kode){
	...
	if (pilih==true) {
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>upliner/hapus",
			data	: string,
			cache	: false,
...
function tampil_data(dat){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>upliner/tambahdata/"+dat,
			data	: string,
			cache	: false,
                beforeSend: function(){
...
<a href="<?php echo base_url();?>upliner"  class="btn btn-primary"> <i class="glyphicon glyphicon-refresh"></i> Refresh</a>
<hr>
  ...
  <script type="text/javascript">
        ...
        $(document).ready(function (){
               jQuery("#list").jqGrid({
              	url		: "<?php echo site_url();?>upliner/loadData",
                		styleUI : 'Bootstrap',
                 mtype : "get",
                datatype: "json",
               colNames:['Userid','Nama Upliner','Aksi'],
                 colModel:[
                   {name:'userid',index:'userid',key: true, width:170, align:"left"},
                    {name:'nm_upliner',index:'nm_upliner',key: true, width:150, align:"left"},
