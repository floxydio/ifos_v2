...
$(document).ready(function(){
	...
	$("#form").hide();
      ...
      $("#approved").click(function(){
	   ...
	   //	var string = "kd_cabang="+kd_cabang+"&nm_cabang="+nm_cabang;
       	var string = "&no_aplikasi="+no_aplikasi+"&pemutus="+pemutus;

               if(pemutus==0){
			//alert("Maaf, Nama Rekening tidak boleh kosong");
			$.messager.show({
				title:'Info',
				msg:'Maaf, Pemutus Tidak Boleh Kosong',
				timeout:2000,
				showType:'fade',
                 style:{
                    right:'',
	...
	if (pilih==true) {
        	$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>approval/updateforward",
			data	: string,
			cache	: false,
    ...
    $("#mymodal").modal("show");
     var link = "approval";
       	...
       	var string = "&no_aplikasi="+no_aplikasi;
                   	...
                   	var pilih = confirm('Anda Yakin Aplikasi akan di Approve');
	if (pilih==true) {
            $.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>parameter/kirimadmin/"+no_aplikasi+"/"+link+"/"+form,
			data	: string,
			cache	: false,
                beforeSend: function(){
 	...
 	$("#cancelm").click(function(){
    	...
    	$("#setujuke").click(function(){


        var no_aplikasi = $("#no_aplikasi").val();
         var limit = $("#limit").val();
          var plafon = $("#plafon").val();
           var setuju =tinymce.get('alasansetuju').getContent(); ;
	   //	var string = "kd_cabang="+kd_cabang+"&nm_cabang="+nm_cabang;
       	var string = "no_aplikasi="+no_aplikasi+"&limit="+limit+"&setuju="+setuju+"&plafon="+plafon;
        	...
        	$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>approval/updatesetuju",
			data	: string,
			cache	: false,
			success	: function(data){
				  tampil_data('<?php echo $aplikasi; ?>','updatereview','approval','formreview','4');
			},
			error : function(xhr, teksStatus, kesalahan) {
				$.messager.show({
					title:'Info',
	   ...
	   //	var string = "kd_cabang="+kd_cabang+"&nm_cabang="+nm_cabang;
       	var string = "&no_aplikasi="+no_aplikasi+"&pemutus="+pemutus;
                      	...
                      	var pilih = confirm('Anda Yakin Aplikasi akan di Analisa Ulang');
	if (pilih==true) {
        	$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>approval/updateanalisa",
			data	: string,
			cache	: false,
			success	: function(data){
				  window.location.assign('<?php echo base_url();?>approval');
...
?>
 ...
 <?php
    ...
    <td  width=150px> <?php echo $listskoring; ?> </td>
      ...
      <tr>
     <td width=100px >Limit</td><td>:</td>
     <?php

		foreach($listlimit->result_array() as $limit){
		?>
    <td  width=150px> <?php echo $limit['jml_limit']; ?> </td>
     <?php
		}
		?>
    </tr>
  ...
  <button type="button" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-user"></i>Keputusan Komite Pembiayaan</button>
   ...
   <td width=100px align="center" >Pemerika Oleh </td><td></td>
    ...
    <td  width=150px></td>
      	...
      	<td width=90px align="center">Tanggal Komite : <?php echo $db['tgl_komite']; ?><br />Disetujui Oleh</td><td></td>
  ...
  $limit =str_replace(",","",$limit['jml_limit']);
		}
   $plafon= str_replace(",","",$db['plafon']);
  if($limit<=$plafon){
     ?>
     <td><button type="button" name="lanjut" id="lanjut" class="btn btn-success btn-sm" disabled><i class="glyphicon glyphicon-send"></i>  Lanjut</button></td>
           <?php
 }else{
     ?>
         <td><button type="button" name="lanjut" id="lanjut" class="btn btn-success btn-sm"><i class="glyphicon glyphicon-send"></i> Lanjut</button></td>

                <?php
 }
     ?>

      <td>    <button type="button" name="approved" id="approved" class="btn btn-success btn-sm"><i class="glyphicon glyphicon-save"></i> Isi Disposisi</button>
   ...
   <td>     <button type="button" name="analisa" id="analisa" class="btn btn-success btn-sm"><i class="glyphicon glyphicon-circle-arrow-bottom"></i> Analisa Ulang</button>
   ...
   <td>     <button type="button" name="canceln" id="canceln" class="btn btn-danger btn-sm"><i class="glyphicon glyphicon-remove"></i> Pembatalan aplikasi</button>
    ...
    <td>        <button type="button" name="tolak" id="tolak" class="btn btn-danger btn-sm"><i class="glyphicon glyphicon-share-alt"></i>Reject</button>
 ...
 <tr>
  ...
  <tr></tr><tr></tr>
   ...
   <tr>
	<td width=20%></td>
    <td>:</td>
    <td><textarea name="alasansetuju" id="alasansetuju"></textarea></td>
    </tr>
 <tr>
     <td>    <button type="button" name="setujuke" id="setujuke" class="btn btn-success btn-sm"><i class="glyphicon glyphicon-save"></i> Simpan</button>


   <button type="button" name="backsetuju" id="backsetuju" class="btn btn-success btn-sm"><i class="glyphicon glyphicon-back"></i> Back</button>
...
<tr>
 ...
 <tr>
  ...
  <tr>
	<td width=20%> Reason :</td>
    <td>:</td>
    <td>   <?php	if($listreason->num_rows() > 0){  ?>
 ...
 <tr>
     <td>  <input type="button" name="simpan" id="tolakke" value="Simpan">

 <input type="button" name="print" id="backtolak" value="Back">
  </td>
  </tr>
  </table>
