...
$tglrealisasi=$rowdetail['tglpenerimaanbarang'];
 if($tglrealisasi==''){
   $tglbaru = $tglakad;
 }else{
...
<div style="'.$tampilpagesttb.'">
<div align="center" class="class15"><strong>SURAT TANDA TERIMA BARANG</strong></div><br /><br />
<div align="justify" class="class14">Pada hari ini '.ucwords(strtolower($hasilhariakad)).', tanggal '.$hasiltglakadt.', bulan '.$hasilbulantglakad.', tahun '.$hasiltglakadtahun.', telah diterima dari PT. Bank syariah Mandiri (selanjutnya disebut BANK) Obyek Akad sebagai pelaksanaan Akad Pembiayaan Dengan Prinsip Murabahah Nomor '.$rowakadnomormrbh['nomor'].'/MRBH tanggal '.$hasiltglakadt.', bulan '.$hasilbulantglakad.', tahun '.$hasiltglakadtahun.' sebagai berikut:</div><br/>';
$dataobjek=mysqli_query($koneksidbi,"SELECT * FROM akad_objek WHERE sha1(kdakaddetailobjek)='$kodedetail' ORDER BY tglinput ASC");
while($rowobjek=mysqli_fetch_array($dataobjek)){

$html.='<table width="541" border="0" cellpadding="0" cellspacing="0">
  <tr>
...
<table width="400" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td width="30"><div align="left" class="class15">NASABAH/PENERIMA BARANG</div></td>
    <td width="5" >&nbsp;</td>
    <td width="300" ><div align="left" class="class15">BANK/YANG MENYERAHKAN BARANG</div></td>
  </tr>
  <tr>
    <td height="70">&nbsp;</td>
    <td >&nbsp;</td>
  ...
  <tr>
    ...
    <td >&nbsp;</td>
    <td ><div align="left" class="class14">'.ucwords(strtolower($rowkuasa['nama_wakil'])).'</div></td>
  </tr>
</table>
</div>
<!-- batas-->
...
//$test='<div style="color:#CCCCCC" class="class14">Akad Murabahah Retail BSM - Dengan Agunan</div>';
//$font=Font_Metrics::get_font("helvetica","bold");
//$canvas->page_text(10,10,$test,"Arial",9,array(0,0,0));
...
$filename=$hasilnamanasabah.'_'.$date.'_akad_murabahah_retail_dengan_agunan'.'.pdf';
$dompdf->stream('"'.$filename.'"', array("Attachment" => 0));
$output=$dompdf->output();

//buat folder 1
	$namafolderbaru="../File_Upload/filepdf/".$koderegakadfoleder;
if(is_dir($namafolderbaru)){
	$simpanfoto=$namafolderbaru;}
	else{
	!mkdir($namafolderbaru);}
//if(file_exists("../akadbawahtangan/filepdf/".$kodereg."/".$filename);
file_put_contents("../File_Upload/filepdf/".$koderegakadfoleder."/".$filename,$output);


mysqli_close($koneksidbi);
mysqli_close($koneksidb);
?>
bah.'_'.$date.'_akad_murabahah_retail_tanpa_agunan'.'.pdf';
$dompdf->stream('"'.$filename.'"', array("Attachment" => 0));
$output=$dompdf->output();
